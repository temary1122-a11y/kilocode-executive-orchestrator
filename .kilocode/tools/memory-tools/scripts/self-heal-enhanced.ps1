...
exit 0
 | ConvertFrom-Json 
                    if ($obj -and $obj.Type -eq 'task.stalled' -and `
                        (($Agent -eq '*') -or (-not $Agent) -or ($obj.Data.Agent -eq $Agent)) -and `
                        [datetime]$obj.Timestamp -ge $cutoff) { 
                        return $obj 
                    } 
                } catch {} 
            } 
...
.Count -ge 2 } | 
        Select-Object -First 1
    if ($stalls) { return @{ TaskId = $stalls.Name; StallCount = $stalls.Count; SampleEvent = $stalls.Group[0] } }
    return @()
}
    # Look for recent stall events (within last 5 minutes) for this agent
    $cutoff = (Get-Date).AddMinutes(-5)
